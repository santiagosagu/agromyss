...
const DetalleProducto = ({ match }) => {
    ...
    useEffect(() => {
        if (productos) {
            const resultado = productos.filter(item => item.url === match.params.producto)
    ...
    useEffect(() => {
        if (productosTodos.length > 0) {
            const resultado = productosTodos[0].productos.filter(item => item.url === match.params.detalle)
                        ...
                        <div className='imagen'>
                            ...
                            >
                                ...
                                {productoActual[0].img.map(image => (
                                    <div className='imagen' style={{ backgroundImage: `url(${image})` }}></div>
                                ))
